...
class CheckTest(unittest.TestCase):
    def test_a_case_the_core_reproduces_reports_nothing(self) -> None:
        self.assertEqual(singlestep.check(case()), [])

    def test_a_register_the_core_gets_wrong_is_named(self) -> None:
        wrong = case()
        wrong["final"]["b"] = 0x42
    ...
    def test_a_byte_of_memory_the_core_gets_wrong_is_named_by_address(self) -> None:
        wrong = case()
    ...
    def test_a_flag_the_case_omits_is_not_compared(self) -> None:
        partial = case()
        del partial["final"]["wz"]

        self.assertEqual(singlestep.check(partial), [])

    def test_a_register_the_case_never_mentions_is_left_holding_what_it_held(self) -> None:
        partial = case()
    ...
    def test_an_interrupt_latch_is_compared_as_a_state_rather_than_a_number(self) -> None:
        latched = case()
...
class PortTest(unittest.TestCase):
    def test_a_read_is_answered_with_what_the_case_says_the_port_gave(self) -> None:
        reading = case(
            name="ED 40",
    ...
    def test_a_transaction_the_core_does_not_perform_is_reported(self) -> None:
        expecting = case()
    ...
    def test_a_port_answers_zero_once_the_case_runs_out_of_answers(self) -> None:
        ports = singlestep.ScriptedPorts([])

        self.assertEqual(ports.read(0x1234), 0)

    def test_a_write_is_recorded_rather_than_answered(self) -> None:
        ports = singlestep.ScriptedPorts([])
...
class OptionTest(unittest.TestCase):
    def test_a_directory_alone_is_enough(self) -> None:
        self.assertEqual(singlestep.options(["suite"]), (Path("suite"), None, None))

    def test_a_limit_bounds_the_cases_read_from_each_file(self) -> None:
        self.assertEqual(singlestep.options(["suite", "--limit", "5"])[1], 5)

    def test_an_opcode_narrows_the_run_to_one_file(self) -> None:
    ...
    def test_a_limit_with_no_number_is_refused(self) -> None:
        with self.assertRaises(singlestep.Usage):
            singlestep.options(["suite", "--limit"])

    def test_an_opcode_with_no_name_is_refused(self) -> None:
        with self.assertRaises(singlestep.Usage):
    ...
    def test_options_with_no_directory_among_them_is_refused(self) -> None:
        with self.assertRaises(singlestep.Usage):
            singlestep.options(["--limit", "5"])


class RunTest(unittest.TestCase):
    def suite(self, cases: list[dict[str, Any]], name: str = "00") -> Path:
        directory = Path(tempfile.mkdtemp())
        (directory / f"{name}.json").write_text(json.dumps(cases))
        return directory

    def test_a_suite_the_core_reproduces_exits_clean(self) -> None:
        directory = self.suite([case()])

        self.assertEqual(singlestep.run([str(directory)]), 0)

    def test_a_suite_with_a_disagreement_does_not(self) -> None:
        wrong = case()
        wrong["final"]["b"] = 0x42
    ...
    def test_naming_an_opcode_that_is_not_there_is_the_same(self) -> None:
        directory = self.suite([case()])

        self.assertEqual(singlestep.run([str(directory), "--opcode", "ff"]), 1)

    def test_a_limit_stops_the_run_reading_the_whole_file(self) -> None:
        wrong = case()
        wrong["final"]["b"] = 0x42
    ...
    def test_more_than_twenty_failures_are_counted_rather_than_all_printed(self) -> None:
        wrong = case()
        wrong["final"]["b"] = 0x42
...
class EntryTest(unittest.TestCase):
    ...
    def test_a_real_run_returns_what_the_run_returned(self) -> None:
        directory = Path(tempfile.mkdtemp())
