...
class SparseTest(unittest.TestCase):
    def test_a_byte_reads_back_as_it_was_written(self):
    ...
    def test_an_address_never_written_still_reads_as_something(self):
        space = memory.SparseMemory(seed=1)

        self.assertIsInstance(space.read8(0x8000), int)

    def test_and_reads_as_the_same_thing_twice(self):
        space = memory.SparseMemory(seed=1)

        self.assertEqual(space.read8(0x8000), space.read8(0x8000))

    def test_an_unwritten_address_is_not_simply_zero_everywhere(self):
        space = memory.SparseMemory(seed=1)

        self.assertTrue(any(space.read8(at) for at in range(0x100)))

    def test_two_spaces_seeded_differently_hold_different_rubbish(self):
    ...
    def test_a_value_wider_than_a_byte_is_narrowed(self):
        space = memory.SparseMemory()
...
class PortTest(unittest.TestCase):
    def test_a_port_never_written_still_answers(self):
        ports = memory.Ports(seed=1)

        self.assertIsInstance(ports.read(0x1234), int)

    def test_a_port_reads_back_what_was_put_there(self):
        ports = memory.Ports()
    ...
    def test_the_address_is_the_whole_sixteen_bits(self):
        ports = memory.Ports()
    ...
    def test_every_transaction_is_recorded_in_order(self):
        ports = memory.Ports()
    ...
    def test_a_read_records_what_it_answered_with(self):
        ports = memory.Ports()
        ports.write(0x1234, 0x42)
        ports.log.clear()
    ...
    def test_the_log_can_be_cleared_between_instructions(self):
        ports = memory.Ports()
        ports.write(0x1234, 0x42)
